...
class InfluenceGraph():
    ...
    def __init__(self, adj_matrix=None, path=None, graph=None):
        ...
        if not (path or skills):
            ...
            self.graph = nx.to_networkx_graph(skills, create_using=nx.DiGraph)
        elif path:
    ...
    def save_edges(self, path, skills):
        """
    ...
    def plot_graph(self, tidy=False, save_path=None):
        ...
        if tidy:
            ...
            ax = nex.draw_networkx(self.graph)
    ...
    def find_paths(self, target):
        ...
        for root in roots:
            results = []
            prereq_bfs(target, self.graph, [root], results)
            if len(results) > 0:
                paths += results
